...
extension Application {
    static func testable(envArgs: [String]? = nil) throws -> Application {
        ...
        let app = try Application(config: config,
                                  environment: env, services: services)
    ...
    func sendRequest<T>(to path: String, method: HTTPMethod, headers: HTTPHeaders = .init(), body: T? = nil) throws -> Response where T: Content {
        ...
        let wrapperRequest = Request(http: request, using: self)
        
        if let body = body {
            try wrapperRequest.content.encode(body)
        }
        
        return try responder.respond(to: wrapperRequest).wait()
    }
